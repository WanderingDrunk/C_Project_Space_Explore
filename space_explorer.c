...
void select_start_and_treasure(Planet *planets, int num_planets, Planet **start, Planet **treasure){
    struct planet_pair *candidates = malloc(sizeof(struct planet_pair)*num_planets*num_planets);
    int num_candidates = 0;
    int i,j;

    // Select randomly between planets farther apart than the width of the space
    for (i=0;i<num_planets;i++){
        for (j=i+1;j<num_planets;j++){
            double dist = get_distance(planets[i].point, planets[j].point);
            if (dist > AREA_SIZE) {
                candidates[num_candidates].first = &planets[i];
                candidates[num_candidates].second = &planets[j];
                num_candidates++;
            }
        }
    }

    // Select a pair of start and treasure randomly
    int cand_id = drand48()*num_candidates;
    struct planet_pair *sel = &candidates[cand_id];
    *start = sel->first;
    *treasure = sel->second;
    free(candidates);
}
...
int main(int argc, char *argv[]) {
    // Define parameters
    double density = POINT_DENSITY;  // Adjust density as needed
    double xlim[2] = {0, AREA_SIZE};  // X-axis limits
    double ylim[2] = {0, AREA_SIZE};  // Y-axis limits
    int num_points;
